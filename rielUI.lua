...
function obj:Window(cfw)
	...
	cfw.DisplayNameDiscord = cfw.DisplayNameDiscord or "@_nmduc0311"
	...
	HideButton.Position = UDim2.new(1, -60, 0, 8)
	HideButton.Size = UDim2.new(0, 22, 0, 22)
	HideButton.Font = Enum.Font.GothamBold
	HideButton.Text = "-"
	HideButton.TextColor3 = Color3.fromRGB(230, 230, 230)
	HideButton.TextSize = 16.000
	HideButton.AutoButtonColor = false
	
	HideFrame = Instance.new("Frame")
	HideFrame.Parent = HideButton
	HideFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
	HideFrame.BackgroundTransparency = 1.000
	HideFrame.BorderSizePixel = 0
	HideFrame.Size = UDim2.new(1, 0, 1, 0)
	
	HideFrameOc = Instance.new("Frame")
	HideFrameOc.Parent = HideButton
	HideFrameOc.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
	HideFrameOc.BackgroundTransparency = 1.000
	HideFrameOc.BorderSizePixel = 0
	HideFrameOc.Size = UDim2.new(1, 0, 1, 0)
	
	obj.Mouseto(HideButton, function()
		HideButton.BackgroundTransparency = 0
		HideFrame.BackgroundTransparency = 0
		HideFrameOc.BackgroundTransparency = 0
	end,function()
	...
	obj.Click(HideButton,function()
		Window11.Visible = false
		spawnHideButton(Window, cfw.KeyCode)
	end)
	...
	DescriptionPlayer.Text = cfw.DiscordDescription
	...
	DescriptionPlayer_2.Text = cfw.DisplayNameDiscord
	...
	DiscordAboutMe.Text = cfw.DiscordDescription
	...
	DiscordMemberSinceReal.Text = cfw.MemberSince or "11 November 2021"
	...
	DescriptionPlayer_3.Text = "Cyndral.dev"
	...
	function JumpTo(TabOder, Name)
		...
		LayersPageLayout:JumpToIndex(TabOder)
	...
	function UITab:AddTab(tabId, cftab)
		if type(tabId) == "table" then
			cftab = tabId
			tabId = nil
		end
		
		cftab = cftab or {}
		cftab.Title = cftab.Title or tabId or "Tab 1"
		...
		local Sec = {}
		function Sec:AddSection(sectionId, ConfigSelection)
			if type(sectionId) == "table" then
				ConfigSelection = sectionId
				sectionId = nil
			end
			
			ConfigSelection = ConfigSelection or {}
			ConfigSelection.Title = ConfigSelection.Title or sectionId or "Section"
			...
			local ScrollLayers_2 = Instance.new("ScrollingFrame")
			...
			ScrollLayers_2.Name = "ScrollLayers"
			ScrollLayers_2.Parent = Layers
			ScrollLayers_2.Active = true
			ScrollLayers_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			ScrollLayers_2.BackgroundTransparency = 0.999
			ScrollLayers_2.BorderColor3 = Color3.fromRGB(0, 0, 0)
			ScrollLayers_2.BorderSizePixel = 0
			ScrollLayers_2.Size = UDim2.new(1, 0, 1, 0)
			ScrollLayers_2.ScrollBarImageColor3 = Color3.fromRGB(0, 0, 0)
			ScrollLayers_2.CanvasSize = UDim2.new(0, 0, 1.20000005, 0)
			ScrollLayers_2.ScrollBarThickness = 0
			ScrollLayers_2.ScrollBarImageTransparency = 0.850
			ScrollLayers_2.LayoutOrder = Counts
			UIListLayout_99.Parent = ScrollLayers_2
			...
			SectionButton.Activated:Connect(function()
				...
				LayersPageLayout:JumpToIndex(ScrollLayers_2.LayoutOrder)
			end)
			...
			local InSec = {}
			function InSec:AddButton(btnId, Mode, ConfigButton)
				if type(btnId) == "string" and type(Mode) == "table" then
					ConfigButton = Mode
					Mode = "Out"
				elseif type(btnId) == "string" and type(Mode) == "string" then
				elseif type(btnId) == "string" then
					Mode = "Out"
				elseif type(btnId) == "table" then
					ConfigButton = btnId
					btnId = nil
					Mode = "Out"
				end
				...
				ConfigButton.Title = ConfigButton.Title or btnId or "Button"
			...
			function InSec:AddToggle(toggleId, Mode, ConfigToggles)
				if type(toggleId) == "string" and type(Mode) == "table" then
					ConfigToggles = Mode
					Mode = "Out"
				elseif type(toggleId) == "string" and type(Mode) == "string" then
				elseif type(toggleId) == "string" then
					Mode = "Out"
				elseif type(toggleId) == "table" then
					ConfigToggles = toggleId
					toggleId = nil
					Mode = "Out"
				end
				...
				ConfigToggles.Title = ConfigToggles.Title or toggleId or "Toggle"
			...
			function InSec:AddDropdown(dropdownId, Mode, ConfigDropdown)
				if type(dropdownId) == "string" and type(Mode) == "table" then
					ConfigDropdown = Mode
					Mode = "Out"
				elseif type(dropdownId) == "string" and type(Mode) == "string" then
					-- Do nothing, parameters are correct
				elseif type(dropdownId) == "string" then
					Mode = "Out"
				elseif type(dropdownId) == "table" then
					ConfigDropdown = dropdownId
					dropdownId = nil
					Mode = "Out"
				end
				
				ConfigDropdown = ConfigDropdown or {}
				ConfigDropdown.Title = ConfigDropdown.Title or dropdownId or "Dropdown"
				ConfigDropdown.Content = ConfigDropdown.Content or ""
				ConfigDropdown.Icon = ConfigDropdown.Logo or ConfigDropdown.Icon or "rbxassetid://7733738683"
				ConfigDropdown.Values = ConfigDropdown.Values or ConfigDropdown.Options or {}
				ConfigDropdown.Multi = ConfigDropdown.Multi or false
				ConfigDropdown.Default = ConfigDropdown.Default or (ConfigDropdown.Multi and {} or "")
				...
				DropdownContent.Text = ConfigDropdown.Content or ConfigDropdown.Description or ""
				...
				DropdownImage.Image = ConfigDropdown.Icon
				...
				function DropFunc:Set(Value)
					...
					if ConfigDropdown.Multi then
						-- For multi-select, ensure Value is a table
					...
					if #DropFunc.Value > 0 then
						DropdownValue = table.concat(DropFunc.Value, ", ")
					end
					OptionSelecting.Text = DropdownValue
					
					if ConfigDropdown.Multi then
						ConfigDropdown.Callback(DropFunc.Value)
					else
						if #DropFunc.Value > 0 then
							ConfigDropdown.Callback(DropFunc.Value[1])
						else
							ConfigDropdown.Callback("")
						end
					end
				end
				...
				function DropFunc:AddValue(NameValue)
					...
					OptionSelecting_2.TextWrapped = true
				...
				print("Values type:", type(ConfigDropdown.Values))
				print("Values count:", type(ConfigDropdown.Values) == "table" and #ConfigDropdown.Values or "not a table")
				if type(ConfigDropdown.Values) == "table" then
					for i, v in ipairs(ConfigDropdown.Values) do
						print("Value " .. i .. ":", v)
					end
				end
			...
			function InSec:AddSlider(sliderId, Mode, ConfigSlider)
				if type(sliderId) == "string" and type(Mode) == "table" then
					ConfigSlider = Mode
					Mode = "Out"
				elseif type(sliderId) == "string" and type(Mode) == "string" then
					-- Do nothing, parameters are correct
				elseif type(sliderId) == "string" then
					Mode = "Out"
				elseif type(sliderId) == "table" then
					ConfigSlider = sliderId
					sliderId = nil
					Mode = "Out"
				end
				
				ConfigSlider = ConfigSlider or {}
				ConfigSlider.Title = ConfigSlider.Title or sliderId or "Slider"
				...
				ConfigSlider.IconSlider = ConfigSlider.Logo or ConfigSlider.IconSlider or "rbxassetid://7733967249"
			...
			function InSec:AddParagraph(paragraphId, Mode, ConfigParagraph)
				if type(paragraphId) == "string" and type(Mode) == "table" then
					ConfigParagraph = Mode
					Mode = "Out"
				elseif type(paragraphId) == "string" and type(Mode) == "string" then
					-- Do nothing, parameters are correct
				elseif type(paragraphId) == "string" then
					Mode = "Out"
				elseif type(paragraphId) == "table" then
					ConfigParagraph = paragraphId
					paragraphId = nil
					Mode = "Out"
				end
				...
				ConfigParagraph.Title = ConfigParagraph.Title or paragraphId or "Paragraph"
				ConfigParagraph.Content = ConfigParagraph.Content or ""
				ConfigParagraph.Logo = ConfigParagraph.Logo or cftab.Icon
				...
				ParagraphImage.AnchorPoint = Vector2.new(0, 0)
				...
				ParagraphImage.Position = UDim2.new(0, 12, 0, 13)
				ParagraphImage.Size = UDim2.new(0, 16, 0, 16)
				ParagraphImage.Image = ConfigParagraph.Logo
				...
				ParagraphTitle.Size = UDim2.new(1, -50, 0, 16)
				...
				ParagraphContent.Position = UDim2.new(0, 40, 0, 31)
				ParagraphContent.Size = UDim2.new(1, -50, 0, 30)
				...
				ParagraphContent.TextSize = 12.000
				ParagraphContent.TextTransparency = 0.400
			...
			function InSec:AddTextBox(textboxId, Mode, ConfigTextBox)
				if type(textboxId) == "string" and type(Mode) == "table" then
					ConfigTextBox = Mode
					Mode = "Out"
				elseif type(textboxId) == "string" and type(Mode) == "string" then
					-- Do nothing, parameters are correct
				elseif type(textboxId) == "string" then
					Mode = "Out"
				elseif type(textboxId) == "table" then
					ConfigTextBox = textboxId
					textboxId = nil
					Mode = "Out"
				end
				...
				ConfigTextBox = ConfigTextBox or {}
				ConfigTextBox.Title = ConfigTextBox.Title or textboxId or "TextBox"
				ConfigTextBox.Content = ConfigTextBox.Content or ""
				ConfigTextBox.Logo = ConfigTextBox.Logo or cftab.Icon
				ConfigTextBox["Placeholder Text"] = ConfigTextBox["Placeholder Text"] or "Enter text..."
				ConfigTextBox.Callback = ConfigTextBox.Callback or function() end
				
				local TextBox = Instance.new("Frame")
				local UICorner = Instance.new("UICorner")
				local TextBoxImage = Instance.new("ImageLabel")
				local TextBoxTitle = Instance.new("TextLabel")
				local TextBoxContent = Instance.new("TextLabel")
				local InputFrame = Instance.new("Frame")
				local UICorner_2 = Instance.new("UICorner")
				local TextInput = Instance.new("TextBox")
				
				TextBox.Name = ConfigTextBox.Title
				TextBox.Parent = FeatureParent
				TextBox.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				TextBox.BackgroundTransparency = 0.970
				TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
				TextBox.BorderSizePixel = 0
				TextBox.Size = UDim2.new(1, -8, 0, 65)
				...
				TextBoxImage.AnchorPoint = Vector2.new(0, 0)
				TextBoxImage.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				TextBoxImage.BackgroundTransparency = 0.999
				TextBoxImage.BorderColor3 = Color3.fromRGB(0, 0, 0)
				TextBoxImage.BorderSizePixel = 0
				TextBoxImage.Position = UDim2.new(0, 12, 0, 13)
				TextBoxImage.Size = UDim2.new(0, 16, 0, 16)
				TextBoxImage.Image = ConfigTextBox.Logo
				
				TextBoxTitle.Name = "TextBoxTitle"
				TextBoxTitle.Parent = TextBox
				TextBoxTitle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				TextBoxTitle.BackgroundTransparency = 0.999
				TextBoxTitle.BorderColor3 = Color3.fromRGB(0, 0, 0)
				TextBoxTitle.BorderSizePixel = 0
				TextBoxTitle.Position = UDim2.new(0, 40, 0, 11)
				TextBoxTitle.Size = UDim2.new(1, -50, 0, 15)
				TextBoxTitle.Font = Enum.Font.GothamBold
				TextBoxTitle.Text = ConfigTextBox.Title
				...
				TextBoxContent.Parent = TextBox
				TextBoxContent.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				TextBoxContent.BackgroundTransparency = 0.999
				TextBoxContent.BorderColor3 = Color3.fromRGB(0, 0, 0)
				TextBoxContent.BorderSizePixel = 0
				TextBoxContent.Position = UDim2.new(0, 40, 0, 25)
				TextBoxContent.Size = UDim2.new(1, -50, 0, 11)
				TextBoxContent.Font = Enum.Font.GothamMedium
				TextBoxContent.Text = ConfigTextBox.Content
				TextBoxContent.TextColor3 = Color3.fromRGB(230, 230, 230)
				TextBoxContent.TextSize = 11.000
				TextBoxContent.TextTransparency = 0.400
				TextBoxContent.TextXAlignment = Enum.TextXAlignment.Left
				...
				InputFrame.BackgroundTransparency = 0.700
				InputFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
				InputFrame.BorderSizePixel = 0
				InputFrame.Position = UDim2.new(0.5, 0, 1, -10)
				...
				TextInput.PlaceholderText = ConfigTextBox["Placeholder Text"]
				TextInput.Text = ""
				TextInput.TextColor3 = Color3.fromRGB(230, 230, 230)
				TextInput.TextSize = 12.000
				TextInput.TextXAlignment = Enum.TextXAlignment.Left
				
				TextInput.FocusLost:Connect(function(enterPressed)
					ConfigTextBox.Callback(TextInput.Text)
				end)
				...
				function TextBoxFuncs:SetText(text)
					TextInput.Text = text or ""
					return TextBoxFuncs
				end
				
				return TextBoxFuncs
			end
			function InSec:AddDiscordInvite(discordId, Mode, ConfigDiscord)
				if type(discordId) == "string" and type(Mode) == "table" then
					ConfigDiscord = Mode
					Mode = "Out"
				elseif type(discordId) == "string" and type(Mode) == "string" then
					-- Do nothing, parameters are correct
				elseif type(discordId) == "string" then
					Mode = "Out"
				elseif type(discordId) == "table" then
					ConfigDiscord = discordId
					discordId = nil
					Mode = "Out"
				end
				...
				ConfigDiscord = ConfigDiscord or {}
				ConfigDiscord.Title = ConfigDiscord.Title or discordId or "JOIN DISCORD"
				ConfigDiscord.Logo = ConfigDiscord.Logo or "rbxassetid://7733755668"
				ConfigDiscord["Name Hub"] = ConfigDiscord["Name Hub"] or "Discord Server"
				ConfigDiscord["Online Members"] = ConfigDiscord["Online Members"] or "1+ Online"
				ConfigDiscord["Total Members"] = ConfigDiscord["Total Members"] or "10+ Members"
				ConfigDiscord.Callback = ConfigDiscord.Callback or function() end
				...
				Discord.Parent = FeatureParent
				Discord.BackgroundColor3 = Color3.fromRGB(32, 34, 37)
				Discord.BorderColor3 = Color3.fromRGB(0, 0, 0)
				Discord.BorderSizePixel = 0
				Discord.Size = UDim2.new(1, -8, 0, 100)
				...
				DiscordLogo.Position = UDim2.new(0, 12, 0, 10)
				DiscordLogo.Size = UDim2.new(0, 30, 0, 30)
				DiscordLogo.Image = ConfigDiscord.Logo
				
				DiscordTitle.Name = "DiscordTitle"
				DiscordTitle.Parent = Discord
				DiscordTitle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				DiscordTitle.BackgroundTransparency = 1.000
				DiscordTitle.BorderColor3 = Color3.fromRGB(0, 0, 0)
				DiscordTitle.BorderSizePixel = 0
				DiscordTitle.Position = UDim2.new(0, 50, 0, 15)
				DiscordTitle.Size = UDim2.new(1, -60, 0, 20)
				DiscordTitle.Font = Enum.Font.GothamBold
				DiscordTitle.Text = ConfigDiscord.Title
				DiscordTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
				DiscordTitle.TextSize = 15.000
				DiscordTitle.TextXAlignment = Enum.TextXAlignment.Left
				
				ServerInfo.Name = "ServerInfo"
				ServerInfo.Parent = Discord
				ServerInfo.BackgroundColor3 = Color3.fromRGB(47, 49, 54)
				ServerInfo.BorderColor3 = Color3.fromRGB(0, 0, 0)
				ServerInfo.BorderSizePixel = 0
				ServerInfo.Position = UDim2.new(0, 12, 0, 50)
				ServerInfo.Size = UDim2.new(1, -24, 0, 40)
				
				UICorner_2.CornerRadius = UDim.new(0, 3)
				UICorner_2.Parent = ServerInfo
				...
				ServerName.Size = UDim2.new(1, -90, 0, 16)
				ServerName.Font = Enum.Font.GothamBold
				ServerName.Text = ConfigDiscord["Name Hub"]
				ServerName.TextColor3 = Color3.fromRGB(255, 255, 255)
				ServerName.TextSize = 13.000
				ServerName.TextXAlignment = Enum.TextXAlignment.Left
				
				ServerStats.Name = "ServerStats"
				ServerStats.Parent = ServerInfo
				ServerStats.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				ServerStats.BackgroundTransparency = 1.000
				ServerStats.BorderColor3 = Color3.fromRGB(0, 0, 0)
				ServerStats.BorderSizePixel = 0
				ServerStats.Position = UDim2.new(0, 10, 0, 20)
				ServerStats.Size = UDim2.new(1, -90, 0, 16)
				ServerStats.Font = Enum.Font.Gotham
				ServerStats.Text = ConfigDiscord["Online Members"] .. " • " .. ConfigDiscord["Total Members"]
				ServerStats.TextColor3 = Color3.fromRGB(185, 187, 190)
				ServerStats.TextSize = 12.000
				ServerStats.TextXAlignment = Enum.TextXAlignment.Left
				
				JoinButton.Name = "JoinButton"
				JoinButton.Parent = ServerInfo
				JoinButton.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
				JoinButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
				JoinButton.BorderSizePixel = 0
				JoinButton.Position = UDim2.new(1, -80, 0, 8)
				JoinButton.Size = UDim2.new(0, 70, 0, 24)
...
function spawnHideButton(WindowInstance, KeyCode)
	...
	ShowButton.TextSize = 14.000
	ShowButton.BackgroundTransparency = 0.2
	ShowButton.Draggable = true
	ShowButton.ZIndex = 9999
	
	UICorner.CornerRadius = UDim.new(0, 8)
	UICorner.Parent = ShowButton
...
function obj:spawnWindow(config)
    local cfw = {}
    
    cfw.Title = "Window 11 UI"
    cfw.Description = ""
    cfw.User = "User"
    cfw.IconUser = ""
    cfw.UseAcrylic = false
    cfw.Size = UDim2.new(0, 600, 0, 420)
    cfw.DiscordStatus = "Cyndral.dev"
    cfw.DisplayNameDiscord = "@_nmduc0311"
    ...
    if config and config.Setup then
        ...
        if config.Setup.Size then
            cfw.Size = config.Setup.Size
        end
    ...
    if config and config.Info and config.Info.Enable then
        ...
        if config.Info.Status then
            cfw.DiscordStatus = config.Info.Status
        end
        
        if config.Info.Username then
            cfw.DisplayNameDiscord = config.Info.Username
        end
        
        if config.Info["About Me"] then
            cfw.DiscordDescription = config.Info["About Me"]
        end
        
        if config.Info.Pronouns then
            cfw.Pronouns = config.Info.Pronouns
        end
        
        if config.Info["Member Since"] then
            cfw.MemberSince = config.Info["Member Since"]
        end
        
        if config.Info["Custom Status"] then
            cfw.CustomStatus = config.Info["Custom Status"]
        end
        
        if config.Info.Activity then
            cfw.Activity = config.Info.Activity
        end
    end
    
    return self:Window(cfw)
end
-----------------------------------------------------------------------
